...
template StrXor(n) {
    ...
    component b2n[n];
    for (var i = 0; i < n; i ++) {
        // Constraints each byte of inputs to be in range [0, 255] as well as decomposing them into bits
        n2b_a[i] = Num2Bits(8);
        n2b_a[i].in <== a[i];
        n2b_b[i] = Num2Bits(8);
        n2b_b[i].in <== b[i];
...
// Spec https://www.ietf.org/archive/id/draft-irtf-cfrg-hash-to-curve-13.html#name-expand_message_xmd
