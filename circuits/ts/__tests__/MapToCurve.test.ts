...
describe('MapToCurve', () => {
    ...
    it('MapToCurve()', async() => {
        ...
        // Step 21 check
        for (let i = 0; i < 4; i ++) {
            const step21_x = BigInt(await getSignalByName(circuit, witness, 'main.x_out[' + i.toString() + ']'))
            expect(step21_x).toEqual(step16_expected_x_array[i])
            const step21_y = BigInt(await getSignalByName(circuit, witness, 'main.y_out[' + i.toString() + ']'))
